        ...
        while True:
            ...
            while True:
                ...
                if goods.empty():
                    ...
                    jishuqi+=1
                    resources.acquire()
                    decoder2,X_1,tst_1,yhat_1=load_model(sess1,data.get_train_data(),data.get_train_label())
                    resources.release()
                result_1, precison_1 = compute_accuracy(data.get_train_data(), data.get_train_label(), sess1,X_1,tst_1,yhat_1, decoder2)
...
class Consumer(threading.Thread):  # 消费者类
    def __init__(self,data,condition,resources, goods, train_x,train_y,count_,sleeptime=1):  # sleeptime=1
        threading.Thread.__init__(self)
        self.cond = condition
        self.goods = goods
        self.sleeptime = sleeptime
        ...
        while True:
            ...
            if self.count_ == 1:
                X_3, tst_3, yhat_3, decoder2_3,e1_value, e2_value, e3_value, e4_value, e5_value, e6_value, e7_value, e8_value = load_model_3(
                    sess3,X_3,tst_3,yhat_3,decoder2_3,encoder2_3, train_data, train_label,train_len)
            else:
...
g = Goods()
g_1=Goods()
data_1=Data(train_x,train_y,batch_size)
data_2=Data(train_x,train_y,batch_size)
...
con = Consumer(data_1,c,resources,g,train_x,train_y,2)
con.start()
